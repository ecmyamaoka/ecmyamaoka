...
class SalesreportController extends AppController
{
	...
	public function api_v1_master_goods()
	{
		...
		$item = array();
		array_push($item,"goods_id");//伝票番号
		array_push($item,"goods_supplier_id");//商品コード
	...
	public function csvwrite($ddata){
		...
		$csv = "商品コード,代表商品コード,商品名,セット,売価,原価,受注数量,受注金額,表示フラグ" . PHP_EOL;
		fputs($f, $csv);
		if(isset($ddata)){
			foreach($ddata as $dt) {
				fputcsv($f, $dt);
	...
	public function totalization3($json){
		...
		foreach ($itemMaster as $item) {
			...
			if($goods_visible_flag==1){
				$calc = array_merge($calc,array('goods_visible_flag'=>'表示'));
			}else{
				$calc = array_merge($calc,array('goods_visible_flag'=>'非表示'));
			}
	...
	public function get_receiveorder_row_search($req)
	{
		...
		array_push($item,"goods_representation_id");//代表商品コード
		array_push($item,"goods_visible_flag");//表示フラグ
