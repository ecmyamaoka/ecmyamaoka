        ...
        <div Class="col-md-6">
            <div Class="row m-1">
                ...
                foreach ($imgres1 as $value) {
                    ...
                    echo '<img src="' . $value['imageUrl'] . '" class="img-thumbnail">';
                    echo '</div>';
                }
                ?>
            </div>
    ...
    <div Class="row margintop20">
        <div Class="col-md-6">
            ...
            if (!empty($res1['result'])) {
                ...
                echo '<th class="text-center">PCキャ</th>';
                echo '<th class="text-center">SPキャ</th>';
                echo '<th class="text-center">PC販売</th>';
                echo '<th class="text-center">PC商品</th>';
                echo '<th class="text-center">SP商品</th>';
                echo '</tr>';
                ...
                foreach ($res1['result'] as $value) {
                    ...
                    echo '<td class="text-right">' . $value['s5'] . '</td>';
                    echo '<td class="text-right">' . $value['s6'] . '</td>';
                    echo '<td class="text-right">' . $value['s2'] . '</td>';
                    echo '<td class="text-right">' . $value['s3'] . '</td>';
                    echo '<td class="text-right">' . $value['s4'] . '</td>';
                    echo '</tr>';
                }
                echo '</Table>';
            }
    ...
    <div Class="row margintop20">
        <div Class="col-md-12">
            <Table Class="table table-sm table-borderless resulttable tablefont">
                ...
                <tr>
                    <th width="5%" class="diff-td ">PCキャ</th>
                    <td width="45%" class="diff-td "><?php echo $arkey->check($res1, 'PC用キャッチコピー') ?></td>
                    <td width="5" bgcolor="#fff"></td>
                    <th width="5%" class="diff-td ">PCキャ</th>
                    <td width="45%" class="diff-td "><?php echo $arkey->check($res2, 'PC用キャッチコピー') ?></td>
                </tr>
                <tr>
                    <th width="5%" class="diff-td table-stripe-td">SPキャ</th>
                    <td width="45%" class="diff-td table-stripe-td"><?php echo $arkey->check($res1, 'モバイル用キャッチコピー') ?></td>
                    <td width="5" bgcolor="#fff"></td>
                    <th width="5%" class="diff-td table-stripe-td">SPキャ</th>
                    <td width="45%" class="diff-td table-stripe-td"><?php echo $arkey->check($res2, 'モバイル用キャッチコピー') ?></td>
                </tr>
                <tr>
                    <th width="5%" class="diff-td">PC販売</th>
                    <td width="45%" class="diff-td"><?php echo $arkey->check($res1, 'PC用販売説明文(テキスト)') ?></td>
                    <td width="5" bgcolor="#fff"></td>
                    <th width="5%" class="diff-td">PC販売</th>
                    <td width="45%" class="diff-td"><?php echo $arkey->check($res2, 'PC用販売説明文(テキスト)') ?></td>
                </tr>
                <tr>
                    <th width="5%" class="diff-td table-stripe-td">PC商品</th>
                    <td width="45%" class="diff-td table-stripe-td"><?php echo $arkey->check($res1, 'PC用商品説明文(テキスト)') ?></td>
                    <td width="5" bgcolor="#fff"></td>
                    <th width="5%" class="diff-td table-stripe-td">PC商品</th>
                    <td width="45%" class="diff-td table-stripe-td"><?php echo $arkey->check($res2, 'PC用商品説明文(テキスト)') ?></td>
                </tr>
                <tr>
                    <th width="5%" class="diff-td">SP商品</th>
                    <td width="45%" class="diff-td"><?php echo $arkey->check($res1, 'スマートフォン用商品説明文(テキスト)') ?></td>
                    <td width="5" bgcolor="#fff"></td>
                    <th width="5%" class="diff-td">SP商品</th>
                    <td width="45%" class="diff-td"><?php echo $arkey->check($res2, 'スマートフォン用商品説明文(テキスト)') ?></td>
                </tr>
