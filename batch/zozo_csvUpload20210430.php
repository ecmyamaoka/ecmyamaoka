<?php
    ...
    class Log {
        ...
        private static function write($message, $echo) {
            ...
            $_cid = "-".self::$c_id;
            $file = fopen("/home/ecmasuser/nextengine/batch/log/stockUpload-{$YMD}{$_cid}.log", "a");
            fwrite($file, $message);
            fclose($file);
        }
        ...
        private static function common_write($message) {
            ...
            $file = fopen("/home/ecmasuser/nextengine/batch/log/stockUpload-{$YMD}.log", "a");
            fwrite($file, $message);
            fclose($file);
        }
        private static function write_another($message, $echo) {
            ...
            $file = fopen("/home/ecmasuser/nextengine/batch/log/token.log", "a");
            fwrite($file, $message);
            fclose($file);
        }
    }
                ...
                while ($retry) {
                    ...
                    if ($add_data["data"]){
                        foreach ($add_data["data"] as $rec) {
                            //比較用の一時テーブルに一行分追加
                            $stock = intval($rec["stock_free_quantity"]) + intval($rec["stock_advance_order_free_quantity"]);
                            $stmt->bind_param("isi", $company["id"], $rec["stock_goods_id"], $stock);

                            // if($rec["stock_goods_id"]=='wn0087-XL-BG'){
                            //     echo $stock;
                            //     var_dump($rec);
                            //     exit();
                            // }
    ...
    function csvExport($stock_all, $setItemList) {
        ...
        foreach ($companyList as $company){
            ...
            $file = fopen("/home/ecmasuser/nextengine/batch/log/{$fn}.csv", "w");
            ...
            if ($stock_rec) {
                foreach($stock_rec as $s_data) {
                    if ($s_data["result"] == "success") {
                        ...
                        foreach ($list as $record) {
                            ...
                            $record['stock_free_quantity'] = intval($record["stock_free_quantity"]) + intval($record["stock_advance_order_free_quantity"]);
                            ...
                            if($record['stock_free_quantity']>0){

                                if($record['stock_free_quantity']>15){
                                    $record['stock_free_quantity']=15;
                                }
                ...
                foreach($setItemList[$i_com] as $setItem) {
                    ...
                    if(strcmp($setItem['set_goods_id'] , $setItem['brand_id'] . '-' . $setItem['cs_goods_id']) !=0){
    ...
    function upload2() {
        ...
        $stockurl ="https://apib.zozo.jp/cooperate/shopstock";
        ...
        foreach($companyList as $company) {
            ...
            if ($company['id'] == 23 ){
                ...
                if ($http->get_LastMethodSuccess() != true) {
                    ...
                    $req->AddHeader('Connection','Keep-Alive');
                    $req->AddHeader('Accept','text/html');
                    $pathToFileOnDisk = '/home/ecmasuser/nextengine/batch/log/' . "{$fileName}-{$company['id']}.csv";
    ...
    function upload_Backup($id) {
        ...
        if (copy("{$folder}log/{$file}.csv", "{$folder}{$copy_folder}/{$file}-{$dateStr_YMDHis}.csv")) {
